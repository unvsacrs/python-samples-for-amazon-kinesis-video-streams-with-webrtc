...
class TransformTrack(MediaStreamTrack):
    ...
    def detect(self, frame):
        ...
        new_frame = VideoFrame.from_ndarray(results[0].plot(), format='bgr24')
...
class KinesisVideoClient:
    def __init__(self, client_id, region, channel_arn, credentials, mode, file_path=None):
        self.client_id = client_id
        self.region = region
    ...
    async def handle_sdp_offer(self, payload, client_id, audio_track, video_track, websocket):
        ...
        if video_track:
            t_video_track = TransformTrack(video_track, client_id, self.mode, yolo_model, self.DCMap)
            self.PCMap[client_id].addTrack(t_video_track)
...
async def main():
    ...
    parser.add_argument('--mode', default='echo', type=str, choices=['echo', 'yolo'], help='Operating mode: "echo" for simple video playback, "yolo" for YOLO object detection')
